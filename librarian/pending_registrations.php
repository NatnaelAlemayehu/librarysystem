...
<html>
	...
	<body>
		<?php
			$query = $con->prepare("SELECT username, name, email, category FROM pending_registrations");
			$query->execute();
			...
			if(isset($_POST['l_confirm']))
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					if(isset($_POST['cb_'.$i]))
					{
						...
						elseif($staff === $row[6]){
							$query = $con->prepare("INSERT INTO staff(username, password, name, email) VALUES(?, ?, ?, ?);");
							$query->bind_param("ssss", $username, $row[1], $row[2], $row[3]);
							if(!$query->execute())
								die(error_without_field("ERROR: Couldn\'t insert values"));
							$members++;
							...
							$query = $con->prepare("INSERT INTO faculty(username, password, name, email) VALUES(?, ?, ?, ?);");
							$query->bind_param("ssss", $username, $row[1], $row[2], $row[3]);
							if(!$query->execute())
								die(error_without_field("ERROR: Couldn\'t insert values"));
							$members++;
